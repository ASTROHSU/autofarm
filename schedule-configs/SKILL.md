...
**原則四：付費牆繞過流程（遇到付費內容時必做）**

遇到 Bloomberg、Reuters、The Information、Fortune 等付費牆來源時：
1. 取得文章的乾淨 URL（移除尾巴的追蹤參數，如 `?srnd=undefined&embedded-checkout=true`）
2. 到 `https://archive.ph/` 搜尋該 URL
3. 如果有存檔，點進去讀取完整全文
4. 如果沒有存檔，再用 WebFetch 嘗試，或退而使用其他免費來源的報導

範例：Matt Levine 的 Bloomberg 專欄
- 先到作者頁 `https://www.bloomberg.com/opinion/authors/ARbTQlRLRjE/matthew-s-levine` 找最新文章
- 取得文章 URL，清掉追蹤參數
- 到 archive.ph 搜尋乾淨 URL → 讀取全文

**幾乎所有付費牆內容都應先試 archive.ph，找不到再說。**

**原則五：驗證流程（每條討論都要跑一遍）**
1. 用 WebSearch 搜尋「事件關鍵字 + 人名或機構名」，找到候選連結
2. 用 WebFetch 讀取連結內容，確認文章確實在說這件事
3. 如果 WebFetch 被擋（付費牆），先用 archive.ph 嘗試繞過
